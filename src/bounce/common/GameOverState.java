...
public class GameOverState extends BasicGameState {
	...
	public void render(GameContainer container, StateBasedGame game,
		...
		for (Bang b : bg.explosions)
			b.render(g);
		g.drawImage(ResourceManager.getImage(ExplorerGameClient.GAMEOVER_BANNER_RSC), 225,
				270);
	...
	public void update(GameContainer container, StateBasedGame game,
		...
		timer -= delta;
		if (timer <= 0)
			game.enterState(ExplorerGameClient.STARTUPSTATE, new EmptyTransition(), new HorizontalSplitTransition() );

		// check if there are any finished explosions, if so remove them
		for (Iterator<Bang> i = ((ExplorerGameClient)game).explosions.iterator(); i.hasNext();) {
			if (!i.next().isActive()) {
				i.remove();
			}
		}
